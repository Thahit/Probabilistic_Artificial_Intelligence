...
class Model(object):
    ...
    def __init__(self):
        ...
        h=2
        l=np.power(h, 2)
        self.kernel = lambda x,y: np.exp(-np.divide(cdist(x,y, 'sqeuclidean'),
                                              l))        
        
        # nugget / alpha term
        self.alpha = 1e-10# e-15 is to small
        self.Cov_matr = None
        self.Xs = None
        self.Ys = None
        self.alpha_=None
    ...
    def make_predictions(self, test_features: np.ndarray) -> typing.Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        #extra
        '''
        cov1_2=self.kernel(self.Xs, test_features)#to not have to compute it several times
        
        
        gp_mean = np.dot(np.dot(np.linalg.inv(self.Cov_matr),
                                           cov1_2).T,
                         self.Ys)
        
        
        
        gp_std = np.subtract(self.kernel(test_features, test_features), 
                             np.dot(np.dot(np.linalg.inv(self.Cov_matr),
                                           cov1_2
                                        ).T,
                                        cov1_2))
        '''
        K_2_1 =self.kernel(test_features,self.Xs)
        gp_mean = np.dot(K_2_1, self.alpha_)
        
        V = scipy.linalg.solve_triangular(
                self.L_, K_2_1.T, lower=True, check_finite=False
            )
        
        gp_std=self.kernel(test_features, test_features)#<K_2_2
        gp_std=gp_std[np.diag_indices_from(gp_std)]
        
        gp_std=gp_std-np.einsum("ij,ji->i", V.T, V)
    ...
    def fitting_model(self, train_GT: np.ndarray,train_features: np.ndarray):
        ...
        """
        self.Xs = train_features
        self.Ys = train_GT
        
        #https://github.com/scikit-learn/scikit-learn/blob/36958fb24/sklearn/gaussian_process/_gpr.py#L24
        
        cov=self.kernel(train_features, train_features)
        cov[np.diag_indices_from(cov)] += self.alpha
        
        self.L_ = scipy.linalg.cholesky(cov, lower=True, check_finite=False)
        self.alpha_ = scipy.linalg.cho_solve(
            (self.L_, True),
            train_features,
            check_finite=False,
        )
