...
class EnsembleTrainer(Framework):
    def __init__(self, dataset_train,
        ...
        self.EnsembleNetworks = [MNISTNet(in_features=28*28,out_features=10) for i in range(self.ensemble_size)]
    
        # Q: do I need a list of optimizers? I think no because
        # network.zero_grad()
        #self.optimizer = None
        self.optimizers = [torch.optim.Adam(self.EnsembleNetworks[i].parameters(), lr=self.learning_rate, weight_decay=0.1) for i in range(self.ensemble_size)]

        # implement a train loader over subsets of data
    ...
    def train(self):
        ...
        for network_idx, network in enumerate(self.EnsembleNetworks):   
            ...
            progress_bar = trange(self.num_epochs)
            for _ in progress_bar:
                for batch_idx, (batch_x, batch_y) in enumerate(self.train_loaders[network_idx]):
                    # batch_x are of shape (batch_size, 784), batch_y are of shape (batch_size,)
                    if batch_idx > int(len(self.train_loaders[network_idx])/self.ensemble_size):
                        break
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        ...
        estimated_probability_list = []
        for network in self.EnsembleNetworks:  
            network.eval() 
            estimated_probability = F.softmax(network(x), dim=1) #normalize outputs from each network
            assert estimated_probability.shape == (x.shape[0], 10)
            estimated_probability_list.append(estimated_probability)
        ...
        # average over networks in ensemble
        for i in range(len(estimated_probability_list)):
            estimated_probability += estimated_probability_list[i]
        estimated_probability /= len(estimated_probability_list)
       
       # renormalize
       # estimated_probability = F.softmax(estimated_probability, dim=1)
        '''
        means = torch.max(estimated_probability,axis=1).mean()
        estimated_one_hot = torch.zeros_like(estimated_probability, dtype=torch.float)
  
        for row_idx, idx in enumerate(torch.argmax(estimated_probability, dim=1)):
            estimated_one_hot[row_idx,idx] = 1

        estimated_probability = estimated_one_hot
        assert estimated_probability.shape == (x.shape[0], 10)  
        '''
        return estimated_probability
