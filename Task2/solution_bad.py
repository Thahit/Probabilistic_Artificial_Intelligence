...
def run_solution(dataset_train: torch.utils.data.Dataset, data_dir: str = os.curdir, eval_data=None, output_dir: str = '/results/'):
    """
    ...
    if not combined_model:
        ...
        if eval_data:
            eval_loader = torch.utils.data.DataLoader(
                eval_data, batch_size=128, shuffle=False, drop_last=False
            )
            print("real eval set")
            evaluate(trainer, eval_loader, data_dir, output_dir)
        else:
            eval_loader = torch.utils.data.DataLoader(
                dataset_train, batch_size=128, shuffle=False, drop_last=False
            )
            print("fake eval set")
        #evaluate(trainer, eval_loader, data_dir, output_dir)
...
# for me, using these just makes everything more abstract
class UnivariateGaussian(ParameterDistribution):# why don't we just always use multivar?
    """
    ...
    def __init__(self, mu: torch.Tensor, sigma: torch.Tensor):
        ...
        assert mu.size() == () and sigma.size() == ()#isn't there always just a size of 1 for both?
        assert sigma > 0
        self.mu = mu
        self.sigma = sigma
    ...
    def log_likelihood(self, values: torch.Tensor) -> torch.Tensor:
        ...
        #don't know if I am missing something or if just this is the solution
...
class MultivariateDiagonalGaussian(ParameterDistribution):
    ...
    def __init__(self, mu: torch.Tensor, rho: torch.Tensor):
        ...
        self.mu = mu
        self.rho = rho
        #self.sp=lambda x:torch.log(1 + torch.exp(x))
        self.sp=nn.Softplus()
    ...
    def sample(self) -> torch.Tensor:
        ...
        # for the Multivariate DiagonalGaussian Gaussian distribution. 
        #sigma = self.sp(self.rho)
        #return torch.distributions.Normal(self.mu, sigma).sample()

        # I think you should be able to do this directly, but...
        standrardnorm = torch.distributions.Normal(0, 1).sample(self.mu.size())
        return torch.add(self.mu, torch.mul(self.sp(self.rho), standrardnorm))
...
class DummyTrainer(Framework):
    ...
    def __init__(self, dataset_train,
        ...
        # HyperpBayesNetarameters and general parameters
        self.batch_size = 128
...
class SelfMadeNetwork(nn.Module):
    def __init__(self,
        ...
        self.model = BayesNet(in_features=in_features, out_features=out_features, bias=False)
        '''
        self.layer1 = None
        self.layer2 = None
        self.layer3 = None
...
class BayesianLayer(nn.Module):
    ...
    def __init__(self, in_features: int, out_features: int, bias: bool = True):
        ...
        self.var = -5    #is the mean of the var       if >0 ->very bad somehow
        self.var_var = .5
        self.random = torch.Generator().manual_seed(69)# to not get different results on the test server, don't know if we want several, for several generated things
        ...
        # why do we even eed this to be a ParameterDistribution
        
        #self.prior = UnivariateGaussian(torch.tensor(self.mean), torch.tensor(self.var))
        ...
        self.weights_prior = MultivariateDiagonalGaussian(
              torch.empty((out_features, in_features)).normal_(mean=self.mean, std=self.mean_var, generator=self.random),
              torch.empty((out_features, in_features)).normal_(mean=self.var, std=self.var_var, generator=self.random)
              )

        if self.use_bias:
            self.bias_prior = MultivariateDiagonalGaussian(
              torch.empty((out_features)).normal_(mean=self.mean, std=self.mean_var, generator=self.random),
              torch.empty((out_features)).normal_(mean=self.var, std=self.var_var, generator=self.random)
              )
        ...
        # I think it is not good to set everything to the same value, so we randomize a bit
        self.weights_var_posterior = MultivariateDiagonalGaussian(
              torch.nn.Parameter(torch.empty((out_features, in_features)).normal_(mean=self.mean, std=self.mean_var, generator=self.random)),
              torch.nn.Parameter(torch.empty((out_features, in_features)).normal_(mean=self.var, std=self.var_var, generator=self.random))
              )
        ...
        if self.use_bias:
            ...
            self.bias_var_posterior = MultivariateDiagonalGaussian(
              torch.nn.Parameter(torch.empty((out_features)).normal_(mean=self.mean, std=self.mean_var, generator=self.random)),
              torch.nn.Parameter(torch.empty((out_features)).normal_(mean=self.var, std=self.var_var, generator=self.random))
              )
            assert isinstance(self.bias_var_posterior, ParameterDistribution)
            assert any(True for _ in self.bias_var_posterior.parameters()), 'Bias posterior must have parameters'
        else:
            self.bias_var_posterior = None# can also just leave out?
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        #print(F.linear(inputs, weights, bias).shape)
...
class BayesNet(nn.Module):
    ...
    def __init__(self, in_features: int, hidden_features: list, out_features: int):
        ...
        self.dropout_p = .2# dropout was in the example so, it might be good
        self.dropout_at_eval = False
        #dropout might mess with with the posterior...
        print("dropout: ", self.dropout_p)
    ...
    def forward(self, x: torch.Tensor) -> typing.Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        for layer in self.layers[:-1]:
            x, log_prior_new, log_variational_posterior_new = layer(x)
            log_prior += log_prior_new
            log_variational_posterior+=log_variational_posterior_new
        ...
        x, log_prior_new, log_variational_posterior_new =self.layers[-1](x)
        log_prior += log_prior_new#don't know if = or +=
        log_variational_posterior += log_variational_posterior_new
...
class SelfTrainer(Framework):
    def __init__(self, dataset_train, print_interval=100,*args, **kwargs):
        """
        Basic Framework for creating your own bayesian neural network trainer.
        """
        self.train_set = dataset_train
        self.num_epochs = 50#don't know if training less would be better(prevent overfitting)
        self.batch_size = 128#smaller batches might be good because more weights are sampled, but bad because less significant
        ...
        self.network=BayesNet(in_features=28*28, hidden_features= hidden_features, out_features=10)
        
        self.train_loader = torch.utils.data.DataLoader(
            dataset_train, batch_size=self.batch_size, shuffle=True, drop_last=True)
        
        
        self.optimizer = torch.optim.Adam(self.network.parameters(), lr=0.02, weight_decay=.001) 
        self.scheduler = torch.optim.lr_scheduler.ExponentialLR(self.optimizer, gamma=.92)
    ...
    def train(self):
        ...
        for i in progress_bar:
            for batch_idx, (batch_x, batch_y) in enumerate(self.train_loader):# batch_x are of shape (batch_size, 784), batch_y are of shape (batch_size,)
    ...
    def predict_probabilities(self, x: torch.Tensor) -> torch.Tensor:
        return self.network.predict_probabilities(x)
...
def main():
    ...
    print("data stats: train x_train: ", x_train.shape, " y_train shape: ", y_train.shape)
    stop=int(np.floor(len(y_train)*.9))
    dataset_train = torch.utils.data.TensorDataset(x_train, y_train)
    
    #dataset_train = torch.utils.data.TensorDataset(x_train[:stop], y_train[:stop])
    #dataset_eval = torch.utils.data.TensorDataset(x_train[stop:], y_train[stop:])
    
    # Run actual solution
    run_solution(dataset_train, data_dir=data_dir, output_dir=output_dir)
    #run_solution(dataset_train, data_dir=data_dir, output_dir=output_dir, eval_data=dataset_eval)
