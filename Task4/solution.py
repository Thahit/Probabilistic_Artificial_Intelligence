...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    layers.append(nn.Linear(sizes[-2], sizes[-1]))# last layer
...
class Actor(nn.Module):
    ...
    def forward(self, obs, act=None):
        ...
        log_prob = self._log_prob_from_distribution(pi, act) if act!=None else None
...
class Critic(nn.Module):
    ...
    def __init__(self, obs_dim, hidden_sizes, activation):
        super().__init__()
        self.v_net = mlp([obs_dim+1] + list(hidden_sizes) + [1], activation)#+1 for action
        print("Critic:\n", self.v_net)
        

    def forward(self, obs, act): # i am learning the q func
        """
        ...
        Returns
        -------
            q: torch.Tensor of shape (n, ), i.e., where n is the number of observations.
                Qval estimate for obs.+act
        """
        #print(obs.shape)
        #print(act.reshape((-1,1)).shape)
        
        return torch.squeeze(self.v_net(torch.cat([obs.reshape((-1, 8)), act.reshape((-1,1))], dim=1)), -1)
...
class VPGBuffer:
    ...
    def __init__(self, obs_dim, act_dim, size, gamma, lam):
        ...
        # extra
        self.critic_buf = np.zeros(size, dtype=np.float32)
        self.critic_later_buf = np.zeros(size, dtype=np.float32)
        self.rew1 = np.zeros(size, dtype=np.float32)
    ...
    def end_traj(self, last_val=0, agent = None):
        """
        ...
        state_diff = self.lam*vals[1:] - vals[:-1]

        to_discount =rews[:-1] +state_diff # rewards + value of new state - value of old state
        self.tdres_buf[path_slice] = discount_cumsum(to_discount, discount=self.lam*self.gamma)
        
        self.critic_buf[path_slice]=vals[:-1]
        self.rew1[path_slice]=rews[:-1]
        self.critic_later_buf[path_slice]=self.lam*vals[1:]
    ...
    def get(self):
        ...
        data = dict(obs = self.obs_buf, act = self.act_buf, ret = self.ret_buf,
                    tdres = self.tdres_buf, logp = self.logp_buf, #extras
                    critic = self.critic_buf, critic2 = self.critic_later_buf, rew1 = self.rew1)
...
class Agent:
    def __init__(self, env, activation=nn.Tanh):
        ...
        hidden_sizes_actor = [64, 64, 64, 64]# 32 layers are shit, more layers -> more good
        hidden_sizes_critic = [64, 64]# state action pairs are harder 
        obs_dim = 8
        self.actor =  Actor(obs_dim, 4, hidden_sizes_actor, activation)
        self.critic  = Critic(obs_dim, hidden_sizes_critic, activation)
    ...
    def step(self, state):
        ...
        v: np.ndarray of (1, )
            The qvalue function at the given state.
        ...
        with torch.no_grad():
            actions = self.actor._distribution(state)
            action = actions.sample()
            
            qvalue = self.critic(state, action)
            logp = self.actor._log_prob_from_distribution(actions, action)
        return int(action), qvalue, logp
...
def train(env, seed=0):
    ...
    #lr
    actor_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(actor_optimizer, gamma=.98)# not very good for actor
    critic_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(critic_optimizer, gamma=.97)
    critic_update_rate = .99 # the learning rate decay between critics
    ...
    for epoch in range(epochs):
        ...
        for t in range(steps_per_epoch):
            ...
            if terminal or timeout or epoch_ended:
                ...
                buf.end_traj(v, agent.critic)
                state, ep_ret, ep_len = agent.env.reset(), 0, 0

        mean_return = np.mean(ep_returns) if len(ep_returns) > 0 else np.nan
        print(f"Epoch: {epoch+1}/{epochs}, mean return {mean_return}")
        if (epoch+1) % 5 == 0:
            #print("current lr critic: ", critic_optimizer.param_groups[0]["lr"])
            print("current lr actor: ", actor_optimizer.param_groups[0]["lr"])
        ...
        #logp_grad = agent.actor._log_prob_from_distribution(agent.actor._distribution(data["obs"]),data["act"])
        pi, log_prob = agent.actor(data["obs"], data["act"])
        #advantage = data["ret"]-data["tdres"]
        advantage = data["tdres"]
        #advantage = data["critic"]
        loss_actor = -(advantage*log_prob).mean() # change
        ...
        critic_lr *= critic_update_rate
        for _ in range(90):
            critic_optimizer.zero_grad()
            #compute a loss for the value function, call loss.backwards() and then
            current_vals = agent.critic(data["obs"], data["act"])

            #loss = (data["rew1"]+data["critic2"]-current_vals).pow(2).mean()
            loss = (data["ret"]-current_vals).pow(2).mean()
            loss.backward()
