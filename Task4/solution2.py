...
def mlp(sizes, activation, output_activation=nn.Identity):
    ...
    for i in range(len(sizes)-2):
        layers[f"hidden_layer_{i+1}"] = nn.Linear(sizes[i], sizes[i+1])
        layers[f"hidden_activation_{i+1}"] = activation()
    layers["output_layer"] = nn.Linear(sizes[-2], sizes[-1])
    layers[f"outpute_activation"] = output_activation()
...
class VPGBuffer:
    ...
    def store(self, obs, act, rew, val, logp):
        ...
        assert self.ptr < self.max_size, "buffer is full"
    ...
    def end_traj(self, last_val=0):
        ...
        # What is this for? And why can I slice ret_buf with this index [large:small]
        # without a negative step? Appears to not be used...
        ...
        # TD_residuals are estimates of advantage function
        
        discounted_deltas = np.zeros_like(self.val_buf[path_slice])
        for i in range(len(self.val_buf[path_slice])):
            discounted_deltas[i] = rews[i]-vals[i]+self.gamma*vals[i+1]
        
        gamma_lam = self.gamma*self.lam

        self.tdres_buf[path_slice] = discount_cumsum(discounted_deltas, discount = gamma_lam)
        ...
        discounted_ret = discount_cumsum(rew, discount = self.gamma)
        self.ret_buf[path_slice] = discounted_ret
...
class Agent:
    ...
    def step(self, state):
        ...
        # why doesn't this work (according instructions)?: 
        #return act.numpy().flatten(), v.numpy().flatten(), logp.numpy().flatten()
        #return act.numpy().flatten()[0], value.numpy().flatten(), logp.numpy().flatten()
        return int(act), value, logp
...
def train(env, seed=0):
    ...
    #agent.actor.logits_net.train(True)
    #agent.critic.v_net.train(True)
    ...
    actor_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(actor_optimizer, gamma=0.98) 
    #critic_lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(critic_optimizer, gamma=0.98)
    ...
    for epoch in range(epochs):
        ...
        data = buf.get()
        # dict_keys(['obs', 'act', 'ret', 'tdres', 'logp'])
        ...
        distr = agent.actor._distribution(data['obs']) 
        logp_grad = agent.actor._log_prob_from_distribution(distr,data['act'])

        # note minus for gradient ascent
        #loss_actor = -(data['ret']*logp_grad).sum() # using discounted return to go
        #loss_actor = -(data['ret']*logp_grad).sum()
        loss_actor = -(data['tdres']*logp_grad).sum() # using GAE (current est. of val function is used)
        loss_actor.backward()
        ...
        for _ in range(100):
            ...
            current_vals = agent.critic(data['obs'])
            loss_critic = nn.MSELoss()
            l = loss_critic(current_vals,data['ret']) # using discounted future rewards as target, see paper
            l.backward()
            critic_optimizer.step()
